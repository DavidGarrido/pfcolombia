...
if(isset($_REQUEST["excluir"]) && soloNumeros($_REQUEST["excluir"]) == 1){
    $buscar_excluir = soloNumeros($_REQUEST["excluir"]);
    $sqlFiltro .= " AND (sat_grupos.satura = 0 || sat_grupos.id IS NULL)";
}
...
*/
$nombreGrafica ="HIJOS DE PAZ - EN DISCIPULADO X GENERACIÓN";
$datos = array();
//
//
$sql = "SELECT
...
if($num > 0)
{
	while($PSN->next_record())
	{
        ...
        if($PSN->f('generacionNumero') == 77 || 'generacionNumero' == 8){
            $genNumero = "EVANGELISMO";
        }
       $texto .=  "| GENERACIÓN ".$genNumero.": ".$PSN->f('discipulado')." ";
		$datos[] = "['GENERACIÓN ".$genNumero."', ".$PSN->f('discipulado')."]";
		$totalProspectos += $PSN->f('discipulado');
        //
	}
}else{
    $varError = 1;
...
<input type="hidden" name="doc" value="graphs_007" />

<div class="form-group">
    <center><h2 class="text-center well col-sm-12">.FILTROS DE BUSQUEDA - HIJOS DE PAZ - EN DISCIPULADO X GENERACIÓN.</h2></center>
</div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            */
            ...
            <label class="control-label col-sm-2" for="idUsuario"><strong>Excluir Red iglesias:</strong></label>
            <div class="col-sm-4"><input type="checkbox" name="excluir" value="1" class="form-control" <?php
                                         if($buscar_excluir == 1){
                                             ?>checked="checked"<?php
                                         }
