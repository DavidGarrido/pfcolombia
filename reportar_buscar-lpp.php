...
LEFT JOIN categorias AS CA ON CA.id = C.idSec ";
    $sql .= " WHERE sat_reportes.rep_tip = 307 ";
    //
    ...
    $sql = "SELECT C.descripcion AS regional, RU.*,sat_reportes.*, U.nombre as nombreUsuario, sat_grupos.nombre as nombreGrupo, tbl_adjuntos.adj_url FROM sat_reportes ";
    $sql .= " LEFT JOIN usuario AS U ON U.id = sat_reportes.idUsuario 
...
LEFT JOIN categorias AS CA ON CA.id = C.idSec ";
    //
    $sql.=" WHERE 1 ".$sqlFiltro." AND sat_reportes.rep_tip = 307 GROUP BY sat_reportes.id ORDER BY C.descripcion, U.nombre ASC";
    ...
    <form name="form" id="form" method="get" class="form-horizontal">
        <input type="hidden" name="doc" value="reportar_buscar-lpp" />
        <div>
                ...
                <tbody>
                    ...
                    if($total_registros > 0)
                    {
                        ...
                        while($PSN1->next_record())
                        {
                            ...
                            ?><tr class='clickable-row' data-href='index.php?doc=subcategoria-lpp&id=<?=$id; ?>' >
                                <!--<td><a href="index.php?doc=subcategoria-lpp&id=<?=$id; ?>"><?=str_pad($id, 6, "0", STR_PAD_LEFT); ?></a></td>//-->
                                <td><?=$regional; ?></td>
                                <td><?= $prision; ?></td>
                                <td><?=$nombreUsuario; ?></td>
