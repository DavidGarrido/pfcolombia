    ...
    */
    ...
    if($_POST["funcion"] == "insertar"){
        ...
        */
        ...
        $comentario  = soloNumeros($_REQUEST["testimonio"]);
        ...
        */
        ...
        $comentario = eliminarInvalidos($_REQUEST["testimonio"]);
        $fechaReporte = eliminarInvalidos($_REQUEST["fechaReporte"]);
        $fechaInicio = eliminarInvalidos($_REQUEST["fechaInicio"]);
        if (isset($_REQUEST['sitioReunion'])) {
            $sitioReunion = soloNumeros($_REQUEST["sitioReunion"]);
...
LEFT JOIN categorias AS C ON C.id = RU.reub_reg_fk";
    ...
    if($PSN1->num_rows() > 0)
    {
        ...
        <div class="form-group">
            <center><input type="button" onClick="window.location.href='index.php?doc=reportar_buscar-lpp'" name="previous" class="previous btn btn-danger" value="Cerrar" /> <br />
        </div>
        <?php
        exit;
    }
    ...
    <form method="post" enctype="multipart/form-data" name="form1" id="form1" class="form-horizontal">
        <h3 class="alert alert-info text-center"><?php
            ...
            <div class="cont-btn cont-flex fl-sbet">
                <div class="item-btn">
                    <?php
                    if ($antId != 0) {?>
                    <a href="index.php?doc=subcategoria-lpp&id=<?=$antId ?>" name="previous" class="previous btn btn-info">Anterior reporte <?=$antId ?></a>
                    <?php } ?>
                </div>
                <div class="item-btn">
                    <a href="index.php?doc=reportar_buscar-lpp" name="previous" class="btn btn-warning">Todos los reportes</a>
                </div>
                <div class="item-btn">
                    <?php
                    if ($sigId != 0) {?>
                    <a href="index.php?doc=subcategoria-lpp&id=<?=$sigId ?>" name="previous" class="previous btn btn-info">Siguiente reporte <?=$sigId ?></a>
                    <?php } ?>
                </div>
            </div>
        <div class="cont-tit">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <strong>N° de patios/pabellones:</strong></label>
                ...
                <script>
                    $(function(){
                        ...
                        $("#adicionarAdd2").on('click',function(){
                            $("#tablaAdd2 tbody tr:eq(0)").clone().removeClass('fila-fijaAdd2').appendTo("#tablaAdd2");
                            var tar2 = $(".act_vin_tar").val();
                            var nom2 = $(".act_vin_nom").val();
                            if (tar2!="" && nom2!="") {
                                total = total + 1;
                ...
                <script>
                    $(function(){
                        ...
                        $("#adicionarAdd3").on('click',function(){
                            $("#tablaAdd3 tbody tr:eq(0)").clone().removeClass('fila-fijaAdd3').appendTo("#tablaAdd3");
                            var tar2 = $(".act_vex_tar").val();
                            var nom2 = $(".act_vex_nom").val();
                            if (tar2!="" && nom2!="") {
                                total = total + 1;
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
            <strong>Número de discípulos que pasaron a ECC:</strong>
        ...
        <div class="cont-btn cont-flex fl-sbet">
            <div class="item-btn">
                <input type="button" onClick="window.location.href='index.php?doc=reportar_buscar-lpp'" name="previous" class="previous btn btn-info" value="Cerrar" />
            </div>
            <div class="item-btn">
...
if($idReporteActual > 0){
    ...
    <div class="container">
        ...
        <div class="row">
            <h2 class="alert alert-success text-center"><a href="index.php?doc=subcategoria-lpp&opc=2&id=<?=$ultimoId; ?>" class="h2">Se ha <?php
            if($idReporteActual == 0){
                echo "creado";
            }
            else{
        ...
        <form method="post" enctype="multipart/form-data" name="form1" id="form1" class="form-horizontal">
            ...
            <fieldset>
                ...
                <div class="form-group">
                    ...
                    <div id="ubicacion"></div>
                    <div class="col-sm-2">
                        <strong>N° de patios/pabellones:</strong>
                        <input name="pabellon" type="number" id="pabellon" maxlength="250" value="<?=$pabellon; ?>" class="form-control" required />
                    </div> 
                </div>
                <div class="cont-btn cont-flex fl-sbet">
        ...
        <!--REGISTRO DE GRADUADOS--->
            <fieldset>
                <div class="cont-tit">
                    <div class="hr"><hr></div>
                    <div class="tit-cen">
                        <h3 class="text-center">REGISTRO DE GRAUADOS</h3>
                        <p>A continuación por favor ingrese los datos requeridos</p>
                    </div>
                    <div class="hr"><hr></div>
                </div>
