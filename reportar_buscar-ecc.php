...
LEFT JOIN categorias AS CA ON CA.id = C.idSec";
    //
    $sql.=" WHERE 1 ".$sqlFiltro." AND sat_reportes.rep_tip = 308 GROUP BY sat_reportes.id ORDER BY C.descripcion, U.nombre ASC";
    ...
    <form name="form" id="form" method="get" class="form-horizontal">
        <input type="hidden" name="doc" value="reportar_buscar-ecc" />
        <div>
            <h3 class="alert alert-info text-center">CONSULTAR REPORTES - CADA COMUNIDAD PARA CRISTO "SATURA" (ECC)</h3>
        </div>
        <div class="cont-tit">
            <div class="hr"><hr></div>
            <div class="tit-cen">
                ...
                <tbody>
                    ...
                    if($total_registros > 0){
                        ...
                        while($PSN1->next_record()){
                            ...
                            ?><tr class='clickable-row' data-href='index.php?doc=subcategoria-ecc&id=<?=$id; ?>' >
                                <!--<td><a href="index.php?doc=subcategoria-ecc&id=<?=$id; ?>"><?=str_pad($id, 6, "0", STR_PAD_LEFT); ?></a></td>//-->
                                ...
                                <td><a href="index.php?doc=subcategoria-ecc&id=<?=$id; ?>"><?=$nombreUsuario; ?></a></td>
                                <td><?=$rep_entr; ?></td>
                                <td><?=$plantador; ?></td>
                                <td><?=$fechaReporte; ?></td>
                                    <td><?=$fechaInicio; ?></td>
