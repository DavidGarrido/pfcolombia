...
//
$sql = "SELECT cotizacion.id, cotizacion.estado, cotizacion.idCliente, cotizacion.contactoNombre, abonos.tipo, abonos.observacion, abonos.fechaCreacion, abonos.valor";
$sql.=" FROM cotizacion, abonos ";
//$sql.=" LEFT JOIN usuario as cliente ON cliente.id = estudiante.idColegio";
$sql.=" WHERE cotizacion.id = ".$varMiId." AND abonos.idCotizacion = cotizacion.id AND abonos.id = '".$varMiAbn."'";
//
$PSN1->query($sql);
...
if($varExito != 1){	
?><form method="post" enctype="multipart/form-data" name="form1" id="form1" action="pop_up.php?doc=pop_abono_edit&id=<?=$varMiId; ?>&abn=<?=$varMiAbn ; ?>" class="form-horizontal">
	
    <div class="form-group">
        <h3 class="text-center well">.EDITAR ABONO.</h3>
    </div>
    ...
    <div class="form-group">
		...
		<?
		/*
		*	TRAEMOS LOS TIPOS DE ABONO
		*/
		$sql = "SELECT * ";
		$sql.=" FROM categorias ";
		$sql.=" WHERE idSec = 22 ORDER BY descripcion asc";
    ...
    <div class="form-group">
        <label class="control-label col-sm-2" for="fechaCreacion">Fecha del abono</label>
        <div class="col-sm-10"><input name="fechaCreacion" type="date"  placeholder="AAAA-MM-DD" value="<?=$fechaCreacion; ?>" class="form-control" /></div>
	</div>

    <div class="form-group">
        <label class="control-label col-sm-2" for="valor">Abono</label>
        <div class="col-sm-10"><input type="number" name="valor" id="valor" value="<?=$abonoValor; ?>" /></div>
	</div>
	
    <div class="form-group">
        <label class="control-label col-sm-2" for="tipo">Observaci&oacute;n</label>
...
function generarForm(){
	if(confirm("Esta accion actualizara el abono de la cotizacion seleccionada, esta seguro que desea continuar?"))
	{
